...
exports.getWorst5_tablo = async (req, res) => {
    try {
        console.log("Worst 5 tablo sorgusu çalıştırılıyor...");
        ...
        const query = `
            ...
            WHERE ichat_ucus.yil = (SELECT MAX(ichat_ucus.yil) FROM ichat_ucus) AND ichat_ucus.ucus_sayisi > 0
            GROUP BY iller.il_id, iller.il_ad, havalimanlari.havalimani_id, havalimanlari.havalimani_ad, ichat_ucus.yil
            ORDER BY ucus_sayisi ASC
            LIMIT 5;
        `;
...
exports.getWorst5_ucusgrafik = async (req, res) => {
    try {
        const query = `
            ...
            ORDER BY toplam_ucus_sayisi ASC
            LIMIT 5;
        `;
        ...
        console.error('Worst 5 uçuş grafiği sorgusunda hata:', error);
        res.status(500).json({ error: 'Veriler alınırken bir hata oluştu.' });
    }
};

//Uçuş sayısı en az olan 5 havalimanı ama ek olarak sıfırdan büyük olanları aldık . Çünkü grafikte tamamen sıfır olanlar olunca grafik anlamsız oluyordu. 
exports.getWorst5_yolcugrafik = async (req, res) => {
    try {
        const query = `
            SELECT ichat_ucus.havalimani_ad, SUM(ichat_ucus.yolcu_sayisi) AS toplam_yolcu_sayisi
            FROM ichat_ucus
            WHERE ichat_ucus.yil = (SELECT MAX(ichat_ucus.yil) FROM ichat_ucus) AND ichat_ucus.yolcu_sayisi > 0
            GROUP BY ichat_ucus.havalimani_ad
            ORDER BY toplam_yolcu_sayisi ASC
            LIMIT 5;
            ; 
        `;
