...
exports.getYillar = async (req, res) => {
    ...
    try {
        const [results] = await db.query(`
            SELECT DISTINCT ichat_ucus.yil
            FROM ichat_ucus
            INNER JOIN havalimanlari ON ichat_ucus.havalimani_id = havalimanlari.havalimani_id
...
exports.getHavalimaniVerileri = async (req, res) => {
    ...
    try {
        const [results] = await db.query(`
            SELECT ichat_ucus.*, iller.il_ad
            FROM ichat_ucus
            INNER JOIN havalimanlari ON ichat_ucus.havalimani_id = havalimanlari.havalimani_id
            INNER JOIN iller ON havalimanlari.il_id = iller.il_id
            WHERE havalimanlari.havalimani_ad = ? AND ichat_ucus.yil = ?
