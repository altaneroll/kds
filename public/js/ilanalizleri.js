...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadIller() {
        try {
            const response = await fetch('/ilanalizleri/iller');
    ...
    async function loadYillar(il, havalimani) {
        try {
            const response = await fetch(`/ilanalizleri/yillar/${encodeURIComponent(il)}/${encodeURIComponent(havalimani)}`);
    ...
    ilSelect.addEventListener('change', () => {
        ...
        fetch(`/ilanalizleri/havalimanlari/${encodeURIComponent(secilenIl)}`)
            .then(response => {
    ...
    yilSelect.addEventListener('change', () => {
        ...
        fetch(`/ilanalizleri/veriler/${encodeURIComponent(secilenHavalimani)}/${encodeURIComponent(secilenYil)}`)
            .then(response => {
    ...
    function createChartContainer() {
        ...
        chartContainer.style.marginBottom = '40px'; // Grafikler arasında daha fazla boşluk
    ...
    function drawMonthlyChart(data) {
        ...
        const aylikVeri = data.filter(d => d.yil === yil).map(d => [d.ay, d.ucus_sayisi]);
        ...
        const options = {
            title: `İç Hat Aylık Uçuş Grafiği (${yil})`,
    ...
    function drawSeasonalChart(data) {
        ...
        const mevsimVeri = mevsimler.map(mevsim => {
            const toplam = data.filter(d => d.yil === yil && d.mevsim === mevsim).reduce((sum, d) => sum + d.ucus_sayisi, 0);
        ...
        const options = {
            title: `İç Hat Mevsimsel Uçuş Grafiği (${yil})`,
    ...
    function drawSeasonalPieChart(data) {
        ...
        const mevsimYolcuVeri = mevsimler.map(mevsim => {
            const toplamYolcu = data
                ...
                .reduce((sum, d) => sum + (d.yolcu_sayisi || 0), 0); // 'yolcu_sayisi' alanını kullan
        ...
        const options = {
            title: `İç Hat Mevsimsel Yolcu Dağılımı (${yil})`,
    ...
    function drawLineChart() {
        fetch('/ilanalizleri/cizgi_grafik_verileri') // Backend'den veri çek
    ...
    function createReport(yil, maxUcus, minUcus, enFazlaUcusAylar, enDusukUcusAylar, chartContainer) {
        ...
        raporMadde1.textContent = `${yil} yılında en fazla uçuş ${maxUcus} uçuş ile ${aylarIsimleriStr1} ayında yapılmıştır.`;
        ...
        raporMadde2.textContent = `${yil} yılında en az uçuş ${minUcus} uçuş ile ${aylarIsimleriStr2} ayında yapılmıştır.`;
        ...
        raporMadde3.textContent = `Gelecek uçuş seferleri, ${aylarIsimleriStr1} ve ${aylarIsimleriStr2} ayları dikkate alınarak planlanmalıdır.`;
    ...
    function createSeasonalReport(yil, maxUcus, minUcus, enFazlaUcusMevsimler, enDusukUcusMevsimler, chartContainer) {
        ...
        raporMadde1.textContent = `${yil} yılında en fazla uçuş ${maxUcus} uçuş ile ${mevsimlerIsimleriStr1} mevsiminde yapılmıştır.`;
        raporListesi.appendChild(raporMadde1);

        // 2. Madde: En az uçuş yapılan mevsimler
        const raporMadde2 = document.createElement('li');
        const mevsimlerIsimleriStr2 = enDusukUcusMevsimler.join(', ');
        raporMadde2.textContent = `${yil} yılında en az uçuş ${minUcus} uçuş ile ${mevsimlerIsimleriStr2} mevsiminde yapılmıştır.`;
        raporListesi.appendChild(raporMadde2);

        // 3. Madde: Gelecek uçuş planlaması için öneri
        const raporMadde3 = document.createElement('li');
        raporMadde3.textContent = `Gelecek uçuş seferleri, ${mevsimlerIsimleriStr1} ve ${mevsimlerIsimleriStr2} mevsimleri dikkate alınarak planlanmalıdır.`;
        raporListesi.appendChild(raporMadde3);
    ...
    function createSeasonalPieReport(yil, mevsimYolcuVeri, chartContainer) {
        ...
        const raporMadde1 = document.createElement('li');
        raporMadde1.textContent = `${yil} yılında en fazla yolcu ${enFazlaYolcuMevsim.toplamYolcu} yolcu ile ${enFazlaYolcuMevsim.mevsim} mevsiminde seyahat edilmiştir.`;
        raporListesi.appendChild(raporMadde1);

        // 2. Madde: En az yolcu yapılan mevsim
        const raporMadde2 = document.createElement('li');
        raporMadde2.textContent = `${yil} yılında en az yolcu ${enAzYolcuMevsim.toplamYolcu} yolcu ile ${enAzYolcuMevsim.mevsim} mevsiminde seyahat edilmiştir.`;
        raporListesi.appendChild(raporMadde2);

        // 3. Madde: Gelecek yolcu planlaması için öneri
        const raporMadde3 = document.createElement('li');
        raporMadde3.textContent = `Gelecek yolcu planlaması, ${enFazlaYolcuMevsim.mevsim} ve ${enAzYolcuMevsim.mevsim} mevsimleri dikkate alınarak düzenlenmelidir.`;
        raporListesi.appendChild(raporMadde3);
    ...
    function createLineChartReport(data, chartContainer) {
        ...
        bilgiItem.textContent =`Özel sefer planlamaları çerçevesinde özellikle grafikteki ${maxData.yil} ve ${minData.yil} yıllarına ait veriler dikkate alınmalıdır.` ;
