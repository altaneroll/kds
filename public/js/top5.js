...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        // Tablo gövdesini seç
        const tableBody = document.getElementById('top5-table');
        tableBody.innerHTML = ''; // Önceki satırları temizle

        // Veriyi API'den al
        const response = await fetch('/top5/veriler');
        if (!response.ok) {
    ...
    async function drawCharts() {
        try {
            ...
            const ucusResponse = await fetch('/top5/ucus-grafik');
            ...
            const yolcuResponse = await fetch('/top5/yolcu-grafik');
